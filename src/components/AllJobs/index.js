...
const salaryRangesList = [
  {
    ...
    label: '10 LAP and above',
  },
  {
    salaryRangeId: '2000000',
    label: '20 LAP and above',
  },
  {
    salaryRangeId: '3000000',
    label: '30 LAP and above',
  },
  {
    salaryRangeId: '4000000',
    label: '40 LAP and above',
  },
]
...
class AllJobs extends Component {
  ...
  onGetJobDetails = async () => {
    ...
    const jwtToken = Cookies.get('jwt_token')
    const {checkboxInputs, radioInput, searchInput} = this.state
    const jobsApiUrl = `https://apis.ccbp.in/jobs?employment_type=${checkboxInputs}&minimum-package=${radioInput}&search=$`
    const optionsJobs = {
      headers: {
        Authorization: `Bearer ${jwtToken}`,
      },
  ...
  onGetInputOption = event => {
    ...
    if (inputNotInList.length === 0) {
      this.setState(
        prevState => ({
          checkboxInput: [...prevState.checkboxInputs, event.target.id],
        }),
        this.onGetJobDetails,
      )
    } else {
